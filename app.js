...
async function analyzeStyle() {
  ...
  try {
    ...
    // Fetch the image and add to FormData
    const response = await fetch(`images/${referenceImage.filename}`);
    const blob = await response.blob();
    formData.append('image', blob, referenceImage.filename);
    formData.append('referenceType', referenceType);
    formData.append('strength', strength.toString());

    const apiResponse = await fetch('http://localhost:3001/api/style/extract', {
      method: 'POST',
      body: formData
    });
    ...
    if (data.success) {
      stylePanelState.extractedStyle = data.data.extractedStyle;
      stylePanelState.autoPrompt = data.data.autoGeneratedPrompt;
      stylePanelState.alternativePrompts = data.data.alternativePrompts;
      stylePanelState.metadata = data.data.metadata;
      stylePanelState.step = 'review';
      renderStylePanel();
    } else {
      stylePanelState.error = data;
      stylePanelState.step = 'error';
      renderStylePanel();
    }
  } catch (error) {
    console.error('Style analysis failed:', error);
    stylePanelState.error = {
      error: 'Style analysis failed',
      message: error.message,
      fallbackOptions: [
        { id: 'describe', label: 'Describe the style yourself', description: 'Enter a text description of the style you want to use' },
        { id: 'direct', label: 'Use as direct reference', description: 'Use the image as a visual reference without style extraction (img2img mode)' },
        { id: 'retry', label: 'Try a different image', description: 'Return to the gallery and select a different reference image' }
      ]
    };
    stylePanelState.step = 'error';
    renderStylePanel();
...
async function generateImage() {
  ...
  try {
    const response = await fetch('http://localhost:3001/api/generate', {
      ...
      body: JSON.stringify({
        prompt: fullPrompt,
        styleParams: { referenceType, strength, metadata },
        subject,
    ...
    if (data.success) {
      stylePanelState.generatedImageUrl = data.data.imageUrl;
      stylePanelState.step = 'result';
      renderStylePanel();
    } else {
      stylePanelState.error = { error: data.error, message: data.message };
      stylePanelState.step = 'error';
      renderStylePanel();
    }
  } catch (error) {
    console.error('Generation failed:', error);
    stylePanelState.error = { error: 'Generation failed', message: error.message };
    stylePanelState.step = 'error';
    renderStylePanel();
  }
}
